...
class SuperAdminController extends Controller
{
    ...
    public function getAllUsers()
    {
        ...
        $users = User::where('verified_user', true)
    ...
    {
        return [
            'total_users' => User::where('verified_user', true)->count(),
            'total_memory_verses' => MemoryBank::count(),
            'total_memorize_later' => MemorizeLater::count(),
            'total_audit_logs' => AuditLog::count(),
            'recent_users' => User::where('verified_user', true)->latest()->take(3)->get(),
            'recent_audit_logs' => AuditLog::with('user')->latest()->take(5)->get(), // Keep for dashboard recent activity
            'active_users_30_days' => User::where('verified_user', true)->where('last_login_date', '>=', Carbon::now()->subDays(30))->count(),
            'active_users_7_days' => User::where('verified_user', true)->where('last_login_date', '>=', Carbon::now()->subDays(7))->count(),
        ];
    }

    private function getChartData(): array
    {
        // User registration over time (last 30 days) - only verified users
